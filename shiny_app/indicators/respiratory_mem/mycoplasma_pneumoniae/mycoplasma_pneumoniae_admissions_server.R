...
altTextServer("mpn_admissions_modal",
              title = "Mycoplasma pneumoniae hospital admissions in Scotland",
              content = tags$ul(tags$li("This is a plot showing the number of Mycoplasma pneumoniae hospital admissions in Scotland."),
...
output$mpn_admissions_table <- renderDataTable({
    ...
    arrange(desc(Date)) %>%
    select(Season, ISOWeek, Admissions = mpn) %>%
...
output$mpn_admissions_plot <- renderPlotly({
  all_pathogen_admissions %>%
    select(Date, Year, ISOWeek, Weekord, Season, Admissions = mpn) %>% 
